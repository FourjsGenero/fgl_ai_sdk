...
PUBLIC FUNCTION (client t_client) set_defaults(
    ...
    LET client.connection.secret_key = aim_common.get_api_key("gemini", "GEMINI_API_KEY")
...
PRIVATE FUNCTION _append_user_content_tool_result(
    ...
    CALL aim_common.assert(tool_name IS NOT NULL,"Missing tool name")
    CALL aim_common.assert(tool_use_id IS NOT NULL,"Missing tool use id")
...
PUBLIC FUNCTION (res t_text_response) get_content_text(
    ...
    CALL aim_common.assert( (x>0), SFMT("Invalid content index: %1",x) )
...
PUBLIC FUNCTION (req t_text_request) remove_system_instruction_part(
    ...
    CALL aim_common.assert( (x>0 AND x<=req.systemInstruction.parts.getLength()),
...
--display "\nCONTINUING CONVERSATION WITH TOOL CALLS:"
    ...
    CALL aim_common.assert((jo_function_call IS NOT NULL),"Expecting functionCall response candidate")
