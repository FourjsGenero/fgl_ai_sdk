...
IMPORT FGL com.fourjs.ai.fgl_ai_sdk.aim_mistral
...
MAIN
    ...
    ELSE
       DISPLAY aim_mistral.get_error_message(s)
       DISPLAY "HTTP post status: ", aim_mistral.get_last_http_post_status()
       DISPLAY "HTTP post description : ", aim_mistral.get_last_http_post_description()
    END IF
    ...
    VAR mycallback aim_mistral.t_tool_function_dispatcher = FUNCTION exec_tools
    CALL request.set_system_message("You are a Math teacher.")
    LET x = request.append_user_message("Use the provided tools to generate the result for a user question.")

    VAR tps1 aim_mistral.t_tool_signature_params = [
    ...
    VAR tps2 aim_mistral.t_tool_signature_params = [
    ...
    VAR tps3 aim_mistral.t_tool_signature_params = [
